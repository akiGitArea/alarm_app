...
class TimeKeeper extends ChangeNotifier {
  ...
  DateTime get totalTime =>
      DateTime.utc(0, 0, 0).add(_studyTime.difference(_playTime));
  String get totalTimeText {
    if (totalTime.isBefore(DateTime.utc(0, 0, 0))) {
      return '-' +
          DateFormat.Hms().format(DateTime.utc(0, 0, 0)
              .add(DateTime.utc(0, 0, 0).difference(totalTime)));
    } else {
  ...
  void _handleTimeIsOver() async {
    if (totalTime != DateTime.utc(0, 0, 0) || _timerMode == TimerMode.Study)
      return;
    stopTimer();
  ...
  void handleOnPaused() {
    ...
    if (_timerMode == TimerMode.Play &&
        totalTime.compareTo(DateTime.utc(0, 0, 0)) > 0) {
      _willStopDuration = _studyTime.difference(_playTime);
      _notificationId =
          notifier.scheduleLocalNotification(_willStopDuration); // 通知をスケジュール
    }
    stopTimer();
  }
  ...
  void handleOnResumed() {
    ...
    if (_timerMode == TimerMode.Play &&
        _willStopDuration.compareTo(Duration.zero) > 0 &&
        _willStopDuration.compareTo(backgroundDuration) < 0) {
      _playTime = _playTime
          .add(totalTime.difference(DateTime.utc(0, 0, 0))); // Play時間を時間を進める
    } else {
